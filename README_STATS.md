...
하는데, 지금 스키마엔 이 컬럼이 없어서 종료할 때마다 `"공부 종료 저장 오류"`가 찍히고
있었을 가능성이 있습니다. 이번 버전은 통계 계산에서 `study_minutes`를 아예 쓰지 않으니
당장은 상관없지만, `end_session()` 자체의 저장 오류는 스키마에 컬럼을 추가하거나
`end_session()`에서 그 필드를 빼는 식으로 별도로 정리하시는 걸 권합니다.
